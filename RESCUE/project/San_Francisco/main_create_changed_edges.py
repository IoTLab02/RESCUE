...
'''
Step 2
Run for iteration_number = 1 to 4
DO NOT run for iteration_number = 0

Arguments:
    -z "Original_fire_probabality_zone.csv" -w "final_edge_weights_directional.csv" -f "directional_flow_only.csv" -p "San Francisco_path_objectives.txt" -e 25 -i 4
'''
if __name__ == "__main__":
    ...
    if itr > 1:
        with open("evacuees_list.pkl", "rb") as f:
    ...
    with open("evacuees_list.pkl", "wb") as f:
    ...
    for (u,v) in edges_with_new_traffic:
        # print(u,v)
        t_ff = traffic_df.loc[(traffic_df["From_Node_ID"] == u) & (traffic_df["To_Node_ID"] == v),"Tff"].values[0]
        flow = traffic_df.loc[
            (traffic_df["From_Node_ID"] == u) & (traffic_df["To_Node_ID"] == v),
            "Flow"].values[0] + edge_traffic_increment[(u,v)]
        capacity = traffic_df.loc[
            (traffic_df["From_Node_ID"] == u) & (traffic_df["To_Node_ID"] == v),
            "Capacity"].values[0]
    ...
    edge_df_updated.to_csv("updated_graph.csv", index=False)
    ins_edges.to_csv("ins_edges.csv", index=False)
    del_edges.to_csv("del_edges.csv", index=False)
    
    if itr==4:
        path_name = "results_" + str(evacuees_num) + "_" + str(itr) + ".csv"
